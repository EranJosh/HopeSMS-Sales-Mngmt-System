...
export default function SoftDeleteDetailDialog({ line, onClose, onSuccess }) {
  ...
  async function handleConfirm() {
    ...
    setSubmitting(true)
    try {
      await softDeleteDetailLine(line.transno, line.prodcode, currentUser)
  ...
  return (
    <Modal title="Confirm Delete Line Item" onClose={onClose} maxWidth="max-w-md">
      ...
      <p className="text-sm mb-6" style={{ color: '#4d7a9e' }}>
        Delete <span className="font-bold" style={{ color: '#c8dff5' }}>{line.prodcode} — {line.description}</span> from transaction <span className="font-bold" style={{ color: '#c8dff5' }}>{line.transno}</span>? This can be recovered later.
      </p>
