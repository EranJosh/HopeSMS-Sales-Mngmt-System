...
export default function EditSaleModal({ sale, onClose, onSuccess }) {
  ...
  async function handleSubmit(e) {
    ...
    setSubmitting(true)
    try {
      await updateSale(sale.transno, { salesdate, custno, empno }, currentUser)
    ...
    <Modal title={`Edit Transaction ${sale.transno}`} onClose={onClose}>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <select required value={custno} onChange={e => setCustno(e.target.value)} className="w-full rounded-lg px-3.5 py-2.5 text-sm outline-none transition-colors duration-150" disabled={loadingOptions}>
            <option value="">{loadingOptions ? 'Loading options...' : 'Select customer'}</option>
        ...
        <div>
          ...
          <select required value={empno} onChange={e => setEmpno(e.target.value)} className="w-full rounded-lg px-3.5 py-2.5 text-sm outline-none transition-colors duration-150" disabled={loadingOptions}>
            <option value="">{loadingOptions ? 'Loading options...' : 'Select employee'}</option>
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          <button type="submit" disabled={submitting || loadingOptions} className="px-4 py-2 text-sm font-bold rounded-lg disabled:opacity-50 transition-colors duration-150 cursor-pointer" style={{ backgroundColor: '#ffd24d', color: '#040810', boxShadow: '0 0 14px rgba(255,210,77,0.2)', fontFamily: "'Rajdhani', sans-serif" }} onMouseEnter={e => { if (!submitting) { e.currentTarget.style.backgroundColor = '#f0c030'; e.currentTarget.style.boxShadow = '0 0 20px rgba(255,210,77,0.35)' } }} onMouseLeave={e => { if (!submitting) { e.currentTarget.style.backgroundColor = '#ffd24d'; e.currentTarget.style.boxShadow = '0 0 14px rgba(255,210,77,0.2)' } }}>
