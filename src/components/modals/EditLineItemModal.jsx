...
export default function EditLineItemModal({ line, onClose, onSuccess }) {
  ...
  async function handleSubmit(e) {
    ...
    setSubmitting(true)
    try {
      await updateDetailLine(line.transno, line.prodcode, { quantity }, currentUser)
      toast.success('Line item updated')
      onSuccess()
      onClose()
    } catch (err) {
      setError(err.message)
      toast.error('Failed to update line item. Please try again.')
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <Modal title={`Edit Line Item - ${line.prodcode}`} onClose={onClose} maxWidth="max-w-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input readOnly value={fmt(line.unitprice)} className="w-full rounded-lg px-3.5 py-2.5 text-sm" style={{ backgroundColor: '#070f1e', border: '1px solid rgba(0,229,255,0.06)', color: '#4d7a9e' }} />
        </div>
        <div>
          <label className={labelClass} style={labelStyle}>Quantity</label>
          <input type="number" required min="0.01" step="0.01" value={quantity} onChange={e => setQuantity(e.target.value)} className="w-full rounded-lg px-3.5 py-2.5 text-sm outline-none transition-colors duration-150" />
        </div>
